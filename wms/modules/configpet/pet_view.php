...
if(isset($_POST['btadd'])){
	$strupdate="Update tb_pet SET typewb='".$_POST['f_status']."',updatetime='".date("c")."' Where id='".$_GET['no']."'";
    ...
    <div class="row" style="border: solid 1px black;padding: 5px">
        <div class="w3-panel w3-border col-md-10">
            <div class="panel-body">
                ...
                <p>ชื่อเจ้าของ : <b style="font-size: 20px"> <?php echo $row['firstname'] .' '. $row['lastname'];?></b></p>
                <p>หมายเลขบัตรประจำตัวประชาชน: <?php echo $row['id_card'];?></p>
                <p>เบอร์โทรศัพท์ : <?php echo $row['telephone'];?></p>
                <!--            <p>ที่อยู่ : --><?php //echo $row['address'];?><!--</p>-->
                <p>ที่อยู่: <?php echo $row['numhome'].' ม.'.$row['moo'].' ซ.'.$row['soi'];?></p>
                <!--            <p>รายละเอียด :</p>-->
                <!--            <blockquote>-->
...
<!--            <p>เรื่อง : --><?php //echo $row['subject'];?><!--</p>-->
            <p>ชื่อสัตว์เลี้ยง : <b style="font-size: 20px"> <?php echo $row['petname'];?></b></p>
            <p>อายุสัตว์เลี้ยง: <?php echo $row['ageyear'].' ปี  '.$row['agemonth'].' เดือน';?></p>
            <p>ประเภทสัตว์เลี้ยง : <?php if($row['pettype']==1) {
                    echo "<td>" . 'สุนัข' . "</td>";
                }
...
<!--            <p>ที่อยู่ : --><?php //echo $row['address'];?><!--</p>-->
            <p>สีสัตว์เลี้ยง/ลักษณะพิเศษ: <?php echo $row['petcolor'];?></p>
            <p>สถานะการทำหมัน: <?php echo $row['petsterilize'];?></p>
            <p>สถานะการฉีดวัคซีน: <?php echo $row['vaccinehis'];?></p>
            <?php
            $date1=date_create($row['vaccinedate']);
            $date2=date_create(date());
            $diff=date_diff($date1,$date2);
            ?>
            <p><b style="font-size: 16px">วันที่ฉีดวัคซีนพิษสุนัขบ้าครั้งล่าสุด : <font color="red"><?php if($row['vaccinehis']=='ฉีดแล้ว') { echo DateThai($row['vaccinedate']);?></font> <?php echo '('.$diff->format("%R%a วันที่แล้ว").')'; }
            else {
                echo "ยังไม่ได้รับการฉีดวัคซีน";
            }
            ?></b></p>
...
<!--            <label style="color: red">*กรุณาตรวจสอบชื่อ เวลา และวันที่จากสลิปให้มีข้อมูลที่ตรงกัน*</label>-->
            ...
            <select id="f_status" name="f_status" class="form-control">
                ...
                <option <?php if($selected == '2'){echo("selected");}?> value="2">ลงทะเบียนสัตว์เลี้ยงเรียบร้อย</option>
            </select>
            <br>
        </div>
