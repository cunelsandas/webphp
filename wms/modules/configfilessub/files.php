...
$pagelen = 20; //จำนวนที่แสดงผลข้อมูลต่อหน้า
	...
	//$sql = "select * from $tablename order by no DESC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
		...
		$sql="Select tb_files_sub.*,tb_submenu.id as fid,tb_submenu.name From tb_files_sub INNER JOIN tb_submenu ON tb_files_sub.filetypeid=tb_submenu.id $offtype order by tb_files_sub.filetypeid,tb_files_sub.no DESC Limit $goto,$pagelen";
