    ...
    <center>
        ...
        <table class="content-table">
            ...
            <tr>
                ...
                <th width="5%" align="center">เข้าชมทั้งหมด</th>
                <th width="5%" align="center">เข้าชมเดือนนี้</th>
                <th width="10%" align="center">วันที่เขียน</th>
            ...
            $sqlcountview = "select * from $tablename LEFT JOIN countview_month ON $tablename.no=countview_month.master_id GROUP BY $tablename.no order by $tablename.datepost DESC Limit $goto,$pagelen";
            ...
            if($totalp==0){
                ...
                while($arr = mysqli_fetch_assoc($Query)){
                    ...
                    echo"<td>&nbsp;".$arr['p_view']."&nbsp;ครั้ง</td>";
                    echo"<td>&nbsp;".$arr['p_view_month']."&nbsp;ครั้ง</td>";
            ...
            $sqlsumview="SELECT sum(p_view) as totalsumview FROM $tablename";
            $rssumview = rsQuery($sqlsumview);

            while ($row = mysqli_fetch_assoc($rssumview))
            {
                $sumviewall =  $row['totalsumview'];
            }

            echo "ยอดเข้าชมของ".$modname.' ทั้งหมด <b style="color:red">'.$sumviewall."</b> ครั้ง";
            ...
            $countviewmonth = "select MAX(p_view) from countview_month where master_id=".$_GET['no'];


//            $first_day_of_last_month = date("Y-m-01",strtotime("-1 month"));   เดือนที่แล้วต้นเดือน
//            $last_day_of_last_month  = date('Y-m-t',strtotime("-1 month"));    เดือนที่แล้วสิ้นเดือน
