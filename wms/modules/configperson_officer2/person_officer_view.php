...
$sql="UPDATE $table SET name='$name',offid='$offid',status='$ac',position='$position',sid='$sid',nolist='$nolist',history='$history',workgroup='$workgroup',workgroupid='$workgroupid',subworkgroupid='$subworkgroupid' Where no='".EscapeValue($_GET['no'])."'";
	$rs=rsQuery($sql);
	if($rs){
...
<tr>
<td>หน่วยงาน</td>
		<td><select class="txt" name="type" onchange="ajaxCall(this.value,'workgroup','tb_officer_workgroup');">
		<?php
...
<td>สายงานหลัก</td>
		<td><select class="txt" name="workgroup" id="workgroup" onchange="ajaxCall(this.value,'subworkgroup','tb_officer_subworkgroup');">
				<?php
					$workgroupname=FindRS("select * from tb_officer_workgroup where id=".$row['workgroupid'],"name");
					echo "<option value=".$row['workgroupid'].">".$workgroupname."</option>";
				?>
		</select>
		</td>
</tr>
<tr>
<td>สายงานย่อย</td>
		<td><select class="txt" name="subworkgroup" id="subworkgroup" >
				<?php
					$workgroupname=FindRS("select * from tb_officer_subworkgroup where id=".$row['subworkgroupid'],"name");
					echo "<option value=".$row['subworkgroupid'].">".$workgroupname."</option>";
				?>
		</select>
		</td>
</tr>
