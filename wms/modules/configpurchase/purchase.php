...
if($type=="addnew"){			 //ตรวจสอบก่อนว่ามีการตั้งค่าของ $_GET['type'] เป็นการเพิ่มข่าวใหม่หรือเปล่า
	...
	if(isset($_GET['del'])){
		...
		$updatetran=UpdateTrans($tablename,'delete',$_SESSION['username'],'ID:'.$_GET['no']);
			echo"<script>window.location.href='main.php?_modid=".$modid."&_mod=".$_GET['_mod']."';</script>";
		}
	}
?>
...
<p>
<?php
	$sqlg="select * from tb_purchase_group order by id ASC";
	$rsg=rsQuery($sqlg);
	if($rsg and mysqli_num_rows($rsg)>0){
		...
		while($gdata=mysqli_fetch_assoc($rsg)){
			...
			$getlink = "<a href=\"main.php?_modid=".$modid."&_mod=".$_GET['_mod']."&page=1&groupid=".$gid."\" title=\"$gname\" class='bt-purchase'>$gname</a>&nbsp;&nbsp;";
		}
	}
?>
</p><br>
...
<?php
	...
	if($groupid==0){
        ...
        $sqlg="select * from tb_purchase_group where id = ".$groupid;
        $rsg=rsQuery($sqlg);
        while($gdata=mysqli_fetch_assoc($rsg)){
            $gname=$gdata['name'];
        }
