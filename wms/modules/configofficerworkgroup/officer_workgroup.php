<script type="text/javascript" src="http://www.itglobal.co.th/js/js01.js"></script>
<script>
...
if(isset($_POST['btsave'])){
	...
	switch($_POST['btsave']){
		...
		case "addnew":
			$sql="insert into $tablename(name,offid,listno) values('$name','$offid','$listno')";
			$alert="<script>alert('บันทึกข้อมูลสำเร็จ');window.location.href='main.php?_modid=".$modid."&_mod=".$_GET['_mod']."';</script>";
			
			break;

		case "edit":
			$sql="update $tablename SET name='$name',offid='$offid',listno='$listno' Where id=$id";
...
<form method="post" action="" name="frm01" id="frm01">
	<table width="90%" class="content-input">
		<tr>
			<td width="20%">หน่วยงาน</td><td><select name="cbotype" id="cbotype" onchange="ajaxShowTable(this.value,'showdata','showworkgroup',<?php echo $modid;?>);">
			...
			<tr>
				<td>ชื่อสายงานหลัก</td><td><input type="text" name="txtname" id="txtname" value="<?php echo $v_name;?>"></td>
			</tr>
			<tr>
