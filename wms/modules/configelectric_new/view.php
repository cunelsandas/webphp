...
$latsql="Select * from $tablename where no=".$_GET['no'];
$latsql2="Select * from $tablename where latitude IS NOT NULL and longitude IS NOT NULL";
...
<?php
	...
	if($_POST['btadd']){
		...
		$stradd="Update $tablename SET name='$name',telephone='$tel',email='$email',add_address='$add_address',add_moo='$add_moo',add_tambol='$add_tambol',add_amphur='$add_amphur',add_province='$add_province',subject='$subject',moo='$moo',fix_with_code='$m',remark='$remark',post_ip='$post_ip',status='$status',active='$ac',result='$result',datefinish='$datefinish',officer='$officer',light='$light',lightno='$lightno',starter='$starter',ballard='$ballard',lamp='$lamp',wired='$wired',other='$other' Where no=".$_GET['no'];
		$rs=rsQuery($stradd);
		if($rs){
...
<form name="form_fifa" id="inputArea" action="" method="POST" enctype="multipart/form-data"  onSubmit="return Chkfrm()">
	<fieldset>
		...
		<input type="text" name="txtno" value="<?php echo $_GET['no'];?> " disabled="disabled"><br>
		<label for="txtsubject" class="label">เรื่อง * </label>
		<input type="text" name="txtsubject" value="<?php echo $subject;?>"/><br>
		...
		<input type="input" name="txtemail" value="<?php echo $email;?>"/><br>
		...
		<input type="text" name="txtadd_moo" value="<?php echo $add_moo;?>"><br>
	...
	<fieldset>
		<legend>รายละเอียด</legend>
			<br>
				ข้าพเจ้ามีความประสงค์ให้<?php echo $customer_name;?> ซ่อมแซมไฟฟ้าสาธารณะ หมู่ที่ *:<?php
					echo "<select name=\"txtmoo\" id=\"txtmoo\" style=\"width:60px;\"><option value=\"$moo\">$moo</option>";
					for($i=1;$i<=10;$i++){
			...
			echo "รหัส UTM : "; $myHome->printUTM(); echo "&nbsp;&nbsp;Latitude : $lat_d&nbsp;&nbsp;Longitude : $long_d<br>";
				
			?>
			<br>

			 <div style="width:750px; border:0px #000 solid;">
            <div id="map_canvas" style="width:850px;height:500px;border:solid black 1px;"></div>
		...
		<br>
			<br>
				...
				<?php
					$sql="select * from tb_officer_onsite";
					$r=rsQuery($sql);
	...
	if($active==0){
			...
			echo "<input type=\"button\" name=\"print\" onclick=\"window.open('modules/report/print_form_gl_01.php?tb=".encode64('tb_electric')."&no=".encode64($_GET['no'])."')\" value=\"พิมพ์คำร้อง\">&nbsp;&nbsp;";
			echo "<input type=\"button\" name=\"print\" onclick=\"window.open('modules/report/print_report_gl_01.php?tb=".encode64('tb_electric')."&no=".encode64($_GET['no'])."')\" value=\"พิมพ์รายงานสรุป\">&nbsp;&nbsp;";
