...
if (isset($_POST['btadd'])) {
    $strupdate = "Update tb_garbage_request_cancle SET typewb='" . $_POST['f_status'] . "',updatetime='" . date("c") . "' Where id='" . $_GET['no'] . "'";
...
$sql = "Select * From tb_garbage_request_cancle Where id='" . $_GET['no'] . "'";
$rs = rsQuery($sql);
...
<div class="container">
    <div class="col-md-10" style="text-align:center;">
        <h2>ข้อมูลขอยกเลิกบริการจัดเก็บขยะมูลฝอย</h2>
    </div>
    <div class="row" style="border: solid 1px black;padding: 5px">
        <div class="w3-panel w3-border col-md-10">
            <div class="panel-body">
                ...
                <p>วันที่ยกเลิกบริการ : <?php echo DateTimeThai($row['datepost']); ?>&nbsp;&nbsp; IP Address
                    : <?php echo $row['ip']; ?></p>
                <!--            <p>เรื่อง : --><?php //echo $row['subject'];?><!--</p>-->
                <p>ชื่อ : <b style="font-size: 20px"> <?php echo $row['name']; ?></b></p>
                <p>เลขบัตรประชาชน: <b style="font-size: 16px"> <?php echo $row['id_card']; ?></b></p>
                <p>เบอร์โทรศัพท์: <?php echo $row['telephone']; ?> </p>
                <p>อีเมล/Line ID : <?php echo $row['email']; ?></p>
                <p>ที่อยู่ : <?php echo $row['address']; ?></p>
                <hr>
                <h5 style="text-decoration: underline">ข้อมูลการขอยกเลิกบริการจัดเก็บขยะมูลฝอย</h5>
                <p>หมู่ที่: <?php echo $row['moo']; ?> </p>
                <p>สาเหตุการยกเลิก: <?php echo $row['reason']; ?> </p>
                <p>ที่อยู่ที่ยกเลิกให้บริการจัดเก็บขยะมูลฝอย: <?php echo $row['placefocus']; ?> </p>
                <!--            <p>รายละเอียด :</p>-->
                <!--            <blockquote>-->
                ...
                <!--                <option -->
                <?php //if($selected == '1'){echo("selected");}?><!-- value="1">ภาษีที่ดินและสิ่งปลูกสร้าง</option>-->
                <!--                <option -->
                <?php //if($selected == '2'){echo("selected");}?><!-- value="2">ภาษีป้าย</option>-->
                <!--            </select>-->
            </div>
        </div>
    </div>
...
<!--            </div>-->
    ...
    <div class="col-md-12" style="text-align:center">
        ...
        href=https://maps.google.com/?saddr=Current+Location&daddr=<?php echo $row['lat'] ?>,<?php echo $row['lng'] ?>
        target=_blank>คลิกเพื่อนำทางใน Google Maps</a>
    </div>
    ...
    if ($rs1->num_rows > 0) {
        ...
        <div class="col-md-10" style="text-align:center;margin-top: 5%">
            <h2>รูปภาพสำนาบัตรประจำตัวประชาชน</h2>
