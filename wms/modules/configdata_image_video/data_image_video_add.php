...
//    }
    ...
    $sql = "INSERT INTO $table(subject,detail1,detail2,datepost,ip,status,img_filter) Values('$subject','$detail','','$datepost','$ip','$ac','$img_filter')";
...
//        }
        ...
        if ($_FILES["frm_img"]["tmp_name"][0] <> "") {
          ...
          for ($i = 0; $i < count($_FILES['frm_img']['name']); $i++) {
                ...
                $newname = $table .'_'. $count .'_'.$i.'_' .$timestamp. '.' . $ext;
...
//                $Status = true;
        ...
        $updatetran = UpdateTrans($table, 'add', $_SESSION['username'],'ID:'.$id. '/' . $foldername . '/' . $filenameFordel.' IP: '.$_SERVER['REMOTE_ADDR'] .' /'. get_browser_name($_SERVER['HTTP_USER_AGENT']));
