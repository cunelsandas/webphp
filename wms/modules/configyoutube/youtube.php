...
if($type=="addnew"){			 //ตรวจสอบก่อนว่ามีการตั้งค่าของ $_GET['type'] เป็นการเพิ่มข่าวใหม่หรือเปล่า
	include"youtube_add.php";
}elseif($type=="view"){	     //ตรวจสอบก่อนว่ามีการตั้งค่าของ $_GET['type'] เป็นการดูรายละเอียดข่าวสารหรือเปล่า
	include"youtube_view.php";
}else{
	if(isset($_GET['status'])){
		$sql="UPDATE $tablename SET active='".$_GET['status']."' Where id='".$_GET['id']."'";
		$rs=rsQuery($sql);
...
?>
<p style="right:10%position:ablolute;"><?php echo"<a href=\"main.php?_modid=".$modid."&_mod=".$_GET['_mod']."&type=addnew\" class='link'>เพิ่มรายการใหม่</a>";?></p>
<center>
...
$Query = rsQuery($sql); //คิวรี่คำสั่ง
	...
	if($totalp==0){
		...
		while($arr = mysqli_fetch_assoc($Query)){
			echo"<tr >";
			echo"<td>&nbsp;".$arr['name']."</td>";
			echo"<td>&nbsp;".thaidate($arr['videoid'])."</td>";
			echo"<td align=\"center\">";
			if($arr['active']=="0"){
				echo"<a href=\"main.php?_modid=".$modid."&_mod=".$_GET['_mod']."&status=1&id=".$arr['id']."\">
<img src=\"../images/component/01.png\" border=\"0\" /></a>";
			}else{
				echo"<a href=\"main.php?_modid=".$modid."&_mod=".$_GET['_mod']."&status=0&id=".$arr['id']."\">
<img src=\"../images/component/02.png\" border=\"0\"  /></a>";
			}
			echo"</td>";
