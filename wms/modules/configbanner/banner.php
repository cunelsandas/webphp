...
<select class="txt" name="type" onchange="window.location.href='main.php?_mod=configbanner&_modid=<?php echo $modid;?>&type='+this.options[this.selectedIndex].value;"><option value="">- - - - กรุณาเลือกประเภทที่ต้องการ - - - -</option>
...
<table class="content-table">
	<thead>
		<tr>
			<th width="30%" class="topleft">&nbsp;รายการ</th>
			<th width="20%" align="center">ประเภท</th>
			<th width="30%" align="center">alt</th>
...
<?php
	...
	if(empty($officertype)){
		...
		$offtype="Where position=$officertype";
		$otype=$officertype;
	}
	$sql = "select id from $tablename $offtype order by position,id ASC"; //คิวรี่ข้อมูล เพื่อหาจำนวน แถว Comment ควร select แค่ ฟิวส์เดียว จะทำให้ทำงานได้ไวกว่า
	$result = rsQuery($sql);
	if($result){
...
//	$sql = "select * from $tablename $offtype order by no DESC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
	$sql="Select tb_banner.*,tb_banner_position.id as oid,tb_banner_position.name as pname From tb_banner INNER JOIN tb_banner_position ON tb_banner.position=tb_banner_position.id $offtype  order by tb_banner.id, tb_banner.listno,tb_banner.id ASC Limit $goto,$pagelen";
...
$Query = rsQuery($sql); //คิวรี่คำสั่ง
	...
	if($totalp==0){
		...
		while($arr =mysqli_fetch_assoc($Query)){
			echo"<tr >";
			echo"<td>&nbsp;(".$arr['listno'].")&nbsp;".$arr['name']."</td>";
			...
			echo"</td>";
			echo"<td align=\"center\"><a href=\"main.php?_modid=".$modid."&_mod=".$_GET['_mod']."&type=view&no=".$arr['id']."\"><img src=\"../images/component/docs_16.gif\" border=\"0\" /></a>&nbsp;&nbsp;&nbsp;<a href=\"main.php?_modid=".$modid."&_mod=".$_GET['_mod']."&del=".$arr['id']."\" onclick=\"return confirm('คุณต้องการลบหัวข้อนี้หรือไม่?');\"><img src=\"../images/component/del_16.gif\" border=\"0\"/></a></td>";
			echo"</tr>";
			$i++;
		}
	}
