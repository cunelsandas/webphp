...
if(isset($_POST['btaddpost'])){
	$dt=date("Y-m-d H:i:s");
	$sql="INSERT INTO tb_wb_subnayok(wid,detail,postby,datepost,ip,status,deleted,createdate) VALUES('".$_GET['wid']."','".EscapeValue($_POST['txtpostadd'])."','".$_SESSION['name']."','$timedate','".$_SERVER['REMOTE_ADDR']."','1','0','$dt')";
	$rs=rsQuery($sql);
	if($rs){
...
//if(isset($_POST['btdelete'])){
	...
	$strDel="Update tb_wb_subnayok SET status='0' Where no='".$subno."'";
	$rsdel=rsQuery($strDel);
	if($rsdel){
			// update table tb_trans บันทึกการเพิ่มข้อมูล
		$updatetran=UpdateTrans('tb_wb_subnayok','delete',$_SESSION['username'],'ID:'.$_GET['subno']);
...
if(isset($_GET['masno'])){
	...
	$strDel="Update tb_wb_nayok SET status='0' Where wid='".$masno."'";
	$rsdel=rsQuery($strDel);
	if($rsdel){
			// update table tb_trans บันทึกการเพิ่มข้อมูล
		$updatetran=UpdateTrans('tb_wb_subnayok','delete',$_SESSION['username'],'ID:'.$_GET['masno']);
...
$sql="Select * From tb_wb_nayok Where wid='".$wid."'";
$rs=rsQuery($sql);
...
$sql="Select * From tb_wb_subnayok Where wid='".$_GET['wid']."' Order by no";
$rs=rsQuery($sql);
