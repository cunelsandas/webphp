...
<?php
	...
	//$table="tb_slideshow";
	//$foldername="/fileupload/slideshow/";
	...
	$slidewidth=FindRS("select * from tb_css where id='1'","slide_width");
	$slideheight=FindRS("select * from tb_css where id='1'","slide_height");

	if($slidewidth<>""){
		$gloSlide_width=$slidewidth;
	}
	if($slideheight<>""){
		$gloSlide_height=$slideheight;
	}
	...
	if($rs){
		...
		for ($i=0;$i<=$file_no;$i++){
			...
			if($file[$i]<>""){
				...
				$uploadimage=resizeimage($images[$i],$newfile[$i],$foldername,$domainname,$gloSlide_width,'false');
...
<p style="margin-left:20px;">ภาพที่อัพล่าสุดจะแสดงก่อนโดยจะแสดงได้ทั้งหมด <?php echo $gloSlideshow_fileno;?> ภาพ ขนาดภาพ กว้าง : <?php echo $gloSlide_width;?> , สูง : <?php echo $gloSlide_height;?></p>
