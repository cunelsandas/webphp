...
<p style="margin-left:10px;">เพิ่มประเภท : <input type="text" class="txt" name="depname" autocomplete="off" style="width:250px;" />&nbsp;<input class="bt" type="submit" name="btadddep" value="เพิ่ม"/></p>
</form>-->
...
<select class="txt" name="type" onchange="window.location.href='main.php?_mod=configtype&_modid=<?php echo $modid;?>&type='+this.options[this.selectedIndex].value;"><option value="">- - - - กรุณาเลือกกลุ่มเมนูที่ต้องการ - - - -</option>
...
<table class="content-table">
		...
		<tr>
			<th width="15%" align="center">กลุ่มเมนู</th>
...
$pagelen = 20; //จำนวนที่แสดงผลข้อมูลต่อหน้า
	...
	$sql = "select no from $tablename $offtype order by groupid ASC,no ASC"; //คิวรี่ข้อมูล เพื่อหาจำนวน แถว Comment ควร select แค่ ฟิวส์เดียว จะทำให้ทำงานได้ไวกว่า
	$result = rsQuery($sql);
	...
	//$sql = "select * from $tablename order by no DESC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
		...
		$sql1="Select * from $tablename $offtype Order by groupid ASC,no DESC Limit $goto,$pagelen";
