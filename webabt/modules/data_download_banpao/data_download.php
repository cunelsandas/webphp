...
echo "<center>";
	if(file_exists("images/".$bannername) and $bannername<>""){
        ...
        echo "<p style='font-weight: bolder;text-decoration: underline;font-size: 16px'>Facebook: <a href='https://www.facebook.com/banpaolampang10/'>องค์การบริหารส่วนตำบลบ้านเป้า จังหวัดลำปาง</a></p>";
			echo "<p style='font-weight: bolder;text-decoration: underline;font-size: 16px'>อีเมล:<a href='mailto:banpao.sao@hotmail.com'>banpao.sao@hotmail.com</a> , <a href='mailto:info@banpao-lampang.go.th'>info@banpao-lampang.go.th</a></p>";
	}
echo "</center>";
...
$pagelen = 20; //จำนวนที่แสดงผลข้อมูลต่อหน้า
	...
	//	$sql="Select tb_download.*,tb_officertype.id as oid,tb_officertype.name From $tablename  INNER JOIN tb_officertype ON tb_download.offid=tb_officertype.id where status='1' and offid='".$_GET['offid']."' order by tb_download.offid,no DESC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
		$sql="Select * From $tablename where status='1' and offid='".$offid."' order by offid ASC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
	}else{
		...
		$sql="Select * From $tablename where status='1' order by offid ASC Limit $goto,$pagelen";
	}
