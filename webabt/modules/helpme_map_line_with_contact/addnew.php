...
if (isset($_POST['sub'])) {
    ...
    if ($_POST['f_subject'] == "") {
        ...
        #EMAIL---------------------------------------------------------------------------------------------------------------------------------------
        $infomer_email = $_POST['f_email'];

        $fm = "admin@itglobal.co.th"; // *** ต้องใช้อีเมล์ @yourdomain.com เท่านั้น  ***
        $to = "info@" . $domainname;  //   **ต้องมี mail ในระบบไม่งั้น error
        $custemail = $infomer_email; // อีเมล์ของผู้ติดต่อที่กรอกผ่านแบบฟอร์ม
        ...
        if (!$mail->Send()) {
            echo "<script>alert('ส่งอีเมล์ผิดพลาด')</script>";
        ...
        echo "<script>alert('ส่งอีเมล์ไปที่หน่วยงานเรียบร้อยแล้ว')</script>";
        echo "<script>console.log('" . $mail->ErrorInfo . "')</script>";
        #EMAIL--------------------------------------------------
        ...
        if ($rs) {
            ...
            $fields = "message=มีคนแจ้งเรื่องร้องเรียนเข้ามาเรื่อง $subj \n สามารถเข้าไปตรวจสอบได้ที่ \n $linkwms";
...
<div class="container" style="text-align: left;">
        ...
        <form name="form_help" id="form_help" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="col-sm-10">
                    ...
                    <?php echo date("Y-m-d"); ?>
                    <input type="hidden" name="f_dateInput" id="f_dateInput" value="<?php echo date("c"); ?>" </div> </div> <div class="form-group">
                    <div class="col-sm-10">
    ...
    <script>
        function initMap() {
            myLatLng = {
                lat:   <?php echo $customer_lat; ?>,
                lng:   <?php echo $customer_lng; ?>
            };
