...
<div class='body-bg_ita'><p class='header_ita'>แบบประเมิน ITA (Integrity and Transparency Assessment)</p><p class='header_ita'><?php echo $customer_name;?></p><br>
<?php
	$sql="select * from tb_ita_menu_2563 Order by listno ASC";
	$rs=rsQuery($sql);
	if($rs){
		$strTable .="<dl >";
		while($data=mysqli_fetch_assoc($rs)){
			...
			$sql_sub="select * from tb_ita_submenu_2563 where menuid=$id Order by listno ASC";
				...
				if($rsSub){
					...
					while($dataSub=mysqli_fetch_assoc($rsSub)){
						...
						$sql_detail="select * from tb_ita_detail_2563 where submenuid=$submenuid Order by listno ASC";
