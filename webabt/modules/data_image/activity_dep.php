...
$dep=EscapeValue(decode64($_GET['dp']));
...
if($no<>""){
	...
	if($result){
		...
		$totalrecords= $num_rows = "0"; //หาจำนวนแถวของขัอมูลทั้งหมด
	}
...
//	$totalp = mysqli_num_rows($Query); // หาจำนวน record ที่เรียกออกมา
	if($totalrecords==0){
		...
		while($arr = mysqli_fetch_assoc($Query)){
			...
			echo"<td valign=\"top\" >";
			if($rs_row > 0){
				echo"<img src=".$foldername.$rs_filename['filename'].">";
			}else{
				echo"<img src=".$foldername.$arr['no']."-1.JPG >";
			}
			echo"</td>";
			echo"<td  valign=\"top\">";
...
if ($page > 1) {
	$back = $page - 1;
	echo "<a href=\"index.php?_mod=".encode64($mod)."&dp=".encode64($dep)."&page=1\" title=\"หน้าแรก First Page\">|<<img src=\"images/bt_first.png\" style=\"width:50px;height:25px;border:0;vertical-align: text-bottom;\" align=top></a>";
	echo "<a href=\"index.php?_mod=".encode64($mod)."&dp=".encode64($dep)."&page=$back\" title=\"ย้อนกลับ Previous Page\"><<<img src=\"images/bt_prev.png\" style=\"width:50px;height:25px;border:0;vertical-align: text-bottom;\"></a>";
	if ($start > 1) { echo "....."; }
}
	$icount=1;
	For ($i=$start ; $i<=$end ; $i++) {
		...
		if ($i == $page ) {
			...
			echo "<a href=\"index.php?_mod=".encode64($mod)."&dp=".encode64($dep)."&page=".$i."\" title=\"ไปหน้าที่ $i\" style=\"color:$bgcolor\">".$i."</a>" ;
		}
	$icount++;
	}
if ($page < $totalpage) {
	...
	echo "<a href=\"index.php?_mod=".encode64($mod)."&dp=".encode64($dep)."&page=$next\" title=\"หน้าต่อไป Next Page\">>><img src=\"images/bt_next.png\" style=\"width:50px;height:25px;border:0;vertical-align: text-bottom;\"></a>";
	echo "<a href=\"index.php?_mod=".encode64($mod)."&dp=".encode64($dep)."&page=$totalpage\" title=\"หน้าสุดท้าย Last Page\">>|<img src=\"images/bt_last.png\" style=\"width:50px;height:25px;border:0;vertical-align: text-bottom;\"></a>";
}
