...
$file_no = 1000;   // กำหนด array จำนวน file ที่ต้องการ  $glo...มาจากไฟล์ connect.ini.php
...
//        }
        $TableName = $table;
        $Dirupload = $gloUploadPath;
        $Dirfoder = $folder;
        $FileName = $TableName . '_';
        $DIR = '../' . $Dirupload . '/' . $Dirfoder . '/';
        $fileimg = [];
        if (isset($_POST['img'])) {
            $fileimg = CreateArrayFileImg($_POST['img'], $FileName, $DIR);
        }
        ...
        if ($file) {
            foreach ($file as $key => $value) {
                if ($value['name'] != '') {
                    $filename = "INSERT INTO filename(tablename,masterid,filename) Values('" . $table . "','" . $id . "','" . $value['name'] . "')";
                    $uppicname = rsQuery($filename);
                }
            }
        }
...
<form name="frmnews" method="POST" action="" enctype="multipart/form-data">
    <table class="content-input">
        ...
        <?php for ($i = 0; $i < $file_no; $i++)://วนลูปสร้าง file control เพื่อรับไฟล์ที่จะทำการอัพโหลด?>
            <tr>
                <td style="text-align: right; padding-right: 10px;">ไฟล์ที่ <?php echo($i + 1); ?> </td>
                <td>
                    <?php echo $fileaa; ?>
                </td>
            </tr>
