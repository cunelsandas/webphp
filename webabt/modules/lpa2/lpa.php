...
<?php
	...
	if($rs){
		...
		while($data=mysqli_fetch_assoc($rs)){
			...
			$sql_sub="select * from tb_lpa_submenu_2563 where menuid=$id Order by listno ASC";
				...
				if($rsSub){
					...
					while($dataSub=mysqli_fetch_assoc($rsSub)){
						...
						$sql_detail="select * from tb_lpa_detail_2563 where submenuid=$submenuid Order by listno ASC";
						$rsDetail=rsQuery($sql_detail);
						$sql_detail2="select * from tb_lpa_detail2_2563 where detailid=$detailid2 Order by listno ASC";
						...
						if($rsDetail){
							...
							while($dataDetail=mysqli_fetch_assoc($rsDetail)){
								...
								$sql_detail2="select * from tb_lpa_detail2_2563 where detailid=$detailid Order by listno ASC";
								$rsDetail2=rsQuery($sql_detail2);
								if($rsDetail2){
									$strTable .="<ul>";
