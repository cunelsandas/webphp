<?php
		$sqlActivity="SELECT * from tb_activity where subject LIKE '".$search."%' ORDER BY no DESC Limit 5";
		
		$sqlNews="SELECT * from tb_news where subject LIKE '".$search."%' ORDER BY no DESC Limit 5";
		$sqlPurchase="SELECT * from tb_purchase where subject LIKE '".$search."%' ORDER BY no DESC Limit 5";
		$sql="select * from (select 'activity' as modname,datepost,no as n_id,subject as n_name,@n:= @n+1  n				
									...
									select 'purchase' as modname,datepost,no as n_id,subject as n_name,@n:= @n+1  n
											from tb_purchase, (SELECT @n := 0) m Where subject LIKE '".$search."%' ) a  order by n DESC Limit 20";
