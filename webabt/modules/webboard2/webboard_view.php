...
if(isset($_GET['subno'])){
	...
	$str="update tb_wb_subnayok set deleted='1' where no='$subno'";
	$rs=rsQuery($str);
...
if(isset($_GET['masno'])){
	...
	$str="update tb_wb_nayok set deleted='1' where wid='$masno'";
	$rs=rsQuery($str);
...
if(isset($_POST['btaddpost'])){
	...
	$sql="INSERT INTO tb_wb_subnayok(wid,detail,postby,datepost,ip,status,deleted,createdate) VALUES('".EscapeValue(decode64($_GET['no']))."','".$postadd."','".$name."','$timedate','".$_SERVER['REMOTE_ADDR']."','1','0','$dt')";
	$rs=rsQuery($sql);
	if($rs){
...
$sql="Select * From tb_wb_nayok Where status='1' AND wid='".EscapeValue(decode64($_GET['no']))."'";
$rs=rsQuery($sql);
...
$sql="Select * From tb_wb_subnayok Where status='1'  AND wid='".EscapeValue(decode64($_GET['no']))."' Order by no DESC";
$rs=rsQuery($sql);
