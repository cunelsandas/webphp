...
<table border="0" width="90%">
		<tr>
		 <?php // พนักงานเจ้าหน้าที่  บล๊อกการแสดง 1
				$i=1;
				$sql="Select * From tb_goverment Where sid='1' And status='1' order by nolist";
				$rs=rsQuery($sql);
				if($rs){
						while($row=mysqli_fetch_assoc($rs)){
							...
							if($row['history'] == ""){
                                ...
                                $history= "<br><a href=\"#\" onclick=\"open_new_window('../modules/popup/history_popup.php?no=".encode64($row['no'])."&tb=".encode64($tablename)."&p=".encode64($mod)."');\"><img src=\"../images/document_icon.png\"></a>";
							}
...
<table border="0" width="90%" align="center">
		<tr>
		 <?php //block2
				$i=1;
				$sql="Select * From tb_goverment Where sid='2' And status='1' order by nolist";
				$rs=rsQuery($sql);
				if($rs){
						while($row=mysqli_fetch_assoc($rs)){
							$filepath=SearchImage($tablename,$row['no'],$foldername,"0");
							if(strlen($row['history'])<10 Or $row['hostory']==""){
                                //$history="";
                                $history=!empty($row['history'])?"<br><a href=\"#\" onclick=\"open_new_window('../modules/popup/history_popup.php?no=".encode64($row['no'])."&tb=".encode64('tb_header')."&p=".encode64('header')."');\"><img src=\"images/document_icon.png\"></a>":"";   //ปิด popup
							}else{
								$history=!empty($row['history'])?"<br><a href=\"#\" onclick=\"open_new_window('../modules/popup/history_popup.php?no=".encode64($row['no'])."&tb=".encode64('tb_header')."&p=".encode64($mod)."');\"><img src=\"images/document_icon.png\"></a>":"";
							}
							echo"<td valign=\"top\" align=\"center\">";
							echo"<table height=\"100%\" border=\"0\">";
							echo"<tr>";
...
<table border="0" width="90%" align="center">
		<tr>
		 <?php // เลขาหรือที่ปรึกษา  บล๊อกการแสดง 3
				$i=1;
				$sql="Select * From tb_goverment Where sid='3' And status='1' order by nolist";
				$rs=rsQuery($sql);
				if($rs){
						while($row=mysqli_fetch_assoc($rs)){
							$filepath=SearchImage($tablename,$row['no'],$foldername,"0");
 ...
 <table border="0" width="90%" align="center">
		<tr>
		 <?php // ที่ปรึกษา  บล๊อกการแสดง 4
				$i=1;
				$sql="Select * From tb_goverment Where sid='4' And status='1' order by nolist";
				$rs=rsQuery($sql);
				if($rs){
						while($row=mysqli_fetch_assoc($rs)){
							$filepath=SearchImage($tablename,$row['no'],$foldername,"0");
    ...
    <table border="0" width="90%" align="center">
        <tr>
            ...
            $i=1;
            $sql="Select * From tb_goverment Where sid='5' And status='1' order by nolist";
            $rs=rsQuery($sql);
            if($rs){
                while($row=mysqli_fetch_assoc($rs)){
                    $filepath=SearchImage($tablename,$row['no'],$foldername,"0");
    ...
    <table border="0" width="90%" align="center">
        <tr>
            ...
            $i=1;
            $sql="Select * From tb_goverment Where sid='6' And status='1' order by nolist";
            $rs=rsQuery($sql);
            if($rs){
                while($row=mysqli_fetch_assoc($rs)){
                    $filepath=SearchImage($tablename,$row['no'],$foldername,"0");
    ...
    <table border="0" width="90%" align="center">
        <tr>
            ...
            $i=1;
            $sql="Select * From tb_goverment Where sid='7' And status='1' order by nolist";
            $rs=rsQuery($sql);
            if($rs){
                while($row=mysqli_fetch_assoc($rs)){
                    $filepath=SearchImage($tablename,$row['no'],$foldername,"0");
