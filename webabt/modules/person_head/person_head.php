 ...
 <table border="0" width="90%" align="center">
    ...
    <table border="0" width="90%" align="center">
        <tr>
            <?php // ที่ปรึกษา  บล๊อกการแสดง 12
            $i=1;
            $sql="Select * From $tablename Where sid='13' And status='1' order by nolist";
            $rs=rsQuery($sql);
            if($rs){
                while($row=mysqli_fetch_assoc($rs)){
                    $filepath=SearchImage($tablename,$row['no'],$foldername,"0");
    ...
    <table border="0" width="90%" align="center">
        <tr>
            <?php // ที่ปรึกษา  บล๊อกการแสดง 12
            $i=1;
            $sql="Select * From $tablename Where sid='14' And status='1' order by nolist";
            $rs=rsQuery($sql);
            if($rs){
                while($row=mysqli_fetch_assoc($rs)){
                    $filepath=SearchImage($tablename,$row['no'],$foldername,"0");
    ...
    <table border="0" width="90%" align="center">
        <tr>
            <?php // ที่ปรึกษา  บล๊อกการแสดง 12
            $i=1;
            $sql="Select * From $tablename Where sid='15' And status='1' order by nolist";
            $rs=rsQuery($sql);
            if($rs){
                while($row=mysqli_fetch_assoc($rs)){
                    $filepath=SearchImage($tablename,$row['no'],$foldername,"0");
