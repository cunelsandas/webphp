...
$folder=FindRS("select * from tb_mod where modtype='$mod'","foldername");
	...
	if($device=="Mobile"){
		echo"<a href='view.php' class='link' target='_blank'>แจ้งปัญหา/เรื่องร้องเรียน</a>";
	}else{
		echo"<a href=\"index.php?_mod=".$_GET['_mod']."&type=".encode64('addnew')." \" class='link'>แจ้งปัญหา/เรื่องร้องเรียน</a>";
	}
	...
	$sql = "select * from $tablename Where status='1' order by id DESC ,datepost DESC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
