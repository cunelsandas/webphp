...
<table width="90%" cellpadding="1" cellspacing="1" border="0" class="tbl-border1" style="margin-bottom:10px;">
    ...
    <?php   echo "<div class=\"view\">เข้าชม <font style='color:red'>" .$row['p_view']. "</font> ครั้ง</div>" ; ?>
	</div>
...
<?php
	...
	while($rs_filename=mysqli_fetch_assoc($rs2)){
        ...
        $sql3= "UPDATE $tablename SET  p_view=$count WHERE no = $p_id";
        $rsview = rsQuery($sql3);

        $sql2= "UPDATE $tablename SET  p_view=$count WHERE no = $p_id";
        $rsview = rsQuery($sql2);
//
