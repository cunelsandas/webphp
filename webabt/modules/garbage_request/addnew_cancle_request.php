...
//            echo "<script>alert('Send Mail OK1')</script>";
            ...
            $sql = "INSERT INTO tb_garbage_request_cancle(id,code,name,id_card,telephone,email,address,province,moo,reason,placefocus,lat,lng,datepost,ip,typewb,status,updatetime,image_key) Values('',2,'".$_POST['f_name']. "','" .$_POST['f_idcard']. "','" .$_POST['f_telephone']. "','" .$_POST['f_email']. "','" .$_POST['f_address']. "','" .$_POST['f_province']. "','" .$_POST['f_moo']. "','" .$_POST['f_reason']. "','" .$_POST['f_placefocus']. "','" .$_POST['f_lat']. "','" .$_POST['f_lng']. "','" .$_POST['f_dateInput']. "','" .$_SERVER['REMOTE_ADDR']. "','1','1','0','".$_POST['f_key']."')";

            $rs = rsQuery($sql);

            if ($rs) {
                echo "<script>alert('แจ้งขอยกเลิกบริการเก็บขยะมูลฝอยเรียบร้อยแล้วค่ะ');window.location.href='index.php?_mod=" . $_GET['_mod'] . "';</script>";//
            }else{
...
//            echo "<script>alert('Send Mail OK')</script>";
            ...
            for($i=0; $i<count($_FILES['f_image']['name']); $i++){
                ...
                $newname = "gar_con_".$_POST['id'].'_'.$i.'.'.$ext;
                $filetmp = $_FILES["f_image"]["tmp_name"][$i];
                $filetype = $_FILES["f_image"]["type"][$i];
                $filepath = "fileupload/garbage_request/cancle/".$newname;
            ...
            $sql = "INSERT INTO tb_garbage_request_cancle(id,code,name,id_card,telephone,email,address,province,moo,reason,placefocus,pole_id,lat,lng,datepost,ip,typewb,status,updatetime,image_key) Values('',2,'".$_POST['f_name']. "','" .$_POST['f_idcard']. "','" .$_POST['f_telephone']. "','" .$_POST['f_email']. "','" .$_POST['f_address']. "','" .$_POST['f_province']. "','" .$_POST['f_moo']. "','" .$_POST['f_reason']. "','" .$_POST['f_placefocus']. "','" .$_POST['f_poleid']. "','" .$_POST['f_lat']. "','" .$_POST['f_lng']. "','" .$_POST['f_dateInput']. "','" .$_SERVER['REMOTE_ADDR']. "','1','1','0','".$_POST['f_key']."')";
            $rs = rsQuery($sql);
            $num = mysqli_fetch_array($rs);
            $lid = $num['id'];
            if ($rs) {
                $sql2 = "SELECT * FROM tb_garbage_request_cancle ORDER BY id DESC LIMIT 0,1";
                $rss = rsQuery($sql2);
                ...
                $fields     = "message=มีผู้แจ้งขอยกเลิกบริการจัดเก็บขยะมูลฝอย ชื่อ $name เลขบัตร $idcard เบอร์โทร $telephone 
                ...
                echo "<script>alert('สามารถตรวจสอบถานะการขอยกเลิกบริการเก็บขยะมูลฝอย เลขที่:" .$lid."');window.location.href='index.php?_mod=" . $_GET['_mod'] . "';</script>";
...
<?php
$sql = "select * from tb_garbage_request_cancle order by id desc LIMIT 1";
$rs = rsQuery($sql);
$rs->num_rows;
$row = $rs->fetch_assoc();
...
<div class="container" style="text-align: left;">
    <div class="col-md-12">
        ...
        <form name="form_help" id="form_help" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="col-sm-10">
                    <label class="control-label"><b>วันที่แจ้งยกเลิกการใช้บริการจัดเก็บขยะมูลฝอย:</b></label>
                    <?php echo date("Y-m-d");?>
            ...
            <div class="form-group row">
                <div class="col-sm-10">
                    ...
                    <input type="text" class="form-control" id="f_address" name="f_address" placeholder="เลขที่ ถนน ซอย หมู่ ตำบล อำเภอ">
                </div>
            </div>
            <div class="form-group row">
                <div class="col-sm-4">
            ...
            <h3 style="font-family: THK2DJuly8;">รายละเอียดสถานที่ยกเลิกการใช้บริการจัดเก็บขยะมูลฝอย</h3>
            <br>
            <h6 style="font-family: THK2DJuly8;">ข้าพเจ้ามีความประสงค์ให้ <?php echo $customer_name ?> ยกเลิกการให้บริการจัดเก็บขยะมูลฝอย</h6>
            <div class="form-group row">
                <div class="col-sm-3">
                ...
                <div class="col-sm-7">
                    <label class="control-label"><b>สาเหตุการยกเลิกบริการจัดเก็บขยะมูลฝอย:</b></label>
                    <input type="text" class="form-control" id="f_reason" name="f_reason" placeholder="สาเหตุการยกเลิก">
                </div>
            </div>
            ...
            <div class="form-group row">
                <div class="col-sm-10">
                    <label class="control-label"><b>ที่อยู่สถานที่ยกเลิกการให้บริการจัดเก็บขยะมูลฝอย</b></label>
                    <input type="text" class="form-control" id="f_placefocus" name="f_placefocus" required placeholder="เลขที่ ถนน ซอย หมู่ ตำบล อำเภอ">
                </div>
            </div>

            <div class="form-group row">
                <div class="col-sm-10">
                    <label class="control-label"><b>เพิ่มรูปถ่ายสำเนาบัตรประจำตัวประชาชนพร้อมรับรองสำเนาของผู้แจ้ง:</b></label>
                    <input type="file"  name="f_image[]" id="f_image[]" multiple>
                    <input type="hidden"  name="f_key" id="f_key" value="garbage_cancle_<?php echo $newid; ?>">
                </div>
            </div>
    ...
    <center><button type="submit" name="sub" class="btn btn-success">ส่งคำขอรับบริการ</button></center>
    </form>
